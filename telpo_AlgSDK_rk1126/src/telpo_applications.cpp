...
namespace telpo_algsdk{
    ...
    telpo_ret_t Telpo_det::init(const std::string &algCfg){
        ...
        if(algCfg_j.find("modelDet") == algCfg_j.end())
        {
        ...
        if(algCfg_j.find("totalLabels") == algCfg_j.end())
        {
        ...
        if(algCfg_j.find("anchors") == algCfg_j.end())
        {
        ...
        if(algCfg_j.find("threshObj") != algCfg_j.end())
        {
            thresh_obj = algCfg_j["threshObj"].get<float>();
        }

        if(algCfg_j.find("threshDet") != algCfg_j.end())
        {
            thresh_det = algCfg_j["threshDet"].get<float>();
        }

        if(algCfg_j.find("threshNMS") != algCfg_j.end())
        {
        ...
        if(rknn_init(&ctx, model_data, model_data_size, 0) < 0)
        {
    ...
    telpo_ret_t Telpo_det::infer(cv::Mat &img, std::vector<telpo_object_t> &Tdets){
        ...
        if (rknn_query(ctx, RKNN_QUERY_IN_OUT_NUM, &io_num, sizeof(io_num)) < 0) {
            printf("rknn_init rknn_query io_num error \n");
            return TELPO_RET_FAIL;
        }
        ...
        inputs[0].pass_through = 0;         //需要type和fmt
        ...
        for (int i = 0; i < io_num.n_output; i++) {
            ...
            {
                std::cout<<"rknn_query output_attrs error"<<std::endl;
                return TELPO_RET_FAIL;
            }

            if (output_attrs[i].qnt_type != RKNN_TENSOR_QNT_AFFINE_ASYMMETRIC || output_attrs[i].type != RKNN_TENSOR_UINT8) 
            {
                std::cout<<"required for a Affine asymmetric u8 quantized rknn model"<<std::endl;
                return TELPO_RET_FAIL;
            }
        }
    ...
    int Telpo_det::process(uint8_t* input, float* anchor, int grid_h, int grid_w, int height, int width, int stride,
                   ...
                   uint32_t zp, float scale)
    {
        ...
        uint8_t thres_i8   = qnt_f32_to_affine(thres, zp, scale);


        thres = unsigmoid(thresh_det);
        uint8_t thres_i8_det =  qnt_f32_to_affine(thres, zp, scale);

        // std::cout<<"\noutput anchors: "<<" zp="<<zp<<" scale="<<scale<<std::endl;
        // std::cout<<"\nthres_i8="<<unsigned(thres_i8)<<" thres_i8_det="<<unsigned(thres_i8_det)<<std::endl;
        ...
        for (int a = 0; a < 3; a++) {
            ...
            for (int j = 0; j < grid_w; j++) {
                ...
                if(box_confidence> tmp_conf) tmp_conf=box_confidence;
                if (box_confidence >= thres_i8) {
                    //std::cout<<"\nhave a box_confidence"<<std::endl;
        ...
        //std::cout<<"max box_confi="<<unsigned(tmp_conf)<<std::endl;
        return validCount;
    }

    int Telpo_det::post_process(uint8_t* input0, uint8_t* input1, uint8_t* input2, int model_in_h, int model_in_w, float conf_threshold,
    ...
    telpo_ret_t Telpo_cls::init(std::string &algCfg)
    {
        ...
        if(algCfg_j.find("modelCls") == algCfg_j.end())
        {
        ...
        if(algCfg_j.find("threshCls") != algCfg_j.end())
        {
            _threshCls = algCfg_j["threshObj"].get<float>();
        }

        if(algCfg_j.find("clsNetH") == algCfg_j.end())
        {
            _netH = algCfg_j["threshObj"].get<int>();
        }

        if(algCfg_j.find("clsNetW") == algCfg_j.end())
        {
        ...
        if(rknn_init(&_ctx, _model_data, model_data_size, 0) < 0)
        {
            std::cout<<"rknn init error"<<std::endl;
            return TELPO_RET_FAIL;
        }
...
//Smoker_det**************************************************************
     ...
     * @return {*}
     */ 
    std::mutex Smoker_det::mtx_t;
    Smoker_det* Smoker_det::instance = nullptr;
    Smoker_det* Smoker_det::getInstance()
    {
        if(instance == nullptr)
        {
            std::unique_lock<std::mutex> lock(mtx_t);
            if(instance == nullptr)
            {
                instance = new Smoker_det();
            }
        }
        return instance;
    }

    telpo_ret_t Smoker_det::init()
    {
        std::string algCfg = getAlgCfg(std::string("smoker.json"));
        if(telp_det.init(algCfg)!=0)
        {
            std::cout<<"Smoker_det init fail!\n";
        }

        return TELPO_RET_SUCCESS;
    }

    telpo_ret_t Smoker_det::process(cv::Mat &img, std::vector<telpo_object_t> &retObjects)
    {
        if(telp_det.infer(img, retObjects)!=0)
        {
            std::cout<<"Smoker_det process fail!\n";
            return TELPO_RET_FAIL;
        }
        return  TELPO_RET_SUCCESS;
    }

    telpo_ret_t Smoker_det::exit()
    {
        return TELPO_RET_SUCCESS;
    }
...
//Caller_det**************************************************************
     ...
     * @return {*}
     */ 
    std::mutex Caller_det::mtx_t;
    Caller_det* Caller_det::instance = nullptr;
    Caller_det* Caller_det::getInstance()
    {
        if(instance == nullptr)
        {
            std::unique_lock<std::mutex> lock(mtx_t);
            if(instance == nullptr)
            {
                instance = new Caller_det();
            }
        }
        return instance;
    }

    telpo_ret_t Caller_det::init()
    {
        std::string algCfg = getAlgCfg(std::string("caller.json"));
        if(telp_det.init(algCfg)!=0)
        {
            std::cout<<"Caller_det init fail!\n";
        }

        return TELPO_RET_SUCCESS;
    }

    telpo_ret_t Caller_det::process(cv::Mat &img, std::vector<telpo_object_t> &retObjects)
    {
        if(telp_det.infer(img, retObjects)!=0)
        {
            std::cout<<"Caller_det process fail!\n";
            return TELPO_RET_FAIL;
        }
        return  TELPO_RET_SUCCESS;
    }

    telpo_ret_t Caller_det::exit()
    {
        return TELPO_RET_SUCCESS;
    }
...
//Ebike_det**************************************************************
    /**
     * @brief: implement of  Ebike_det member function
     * @details: 
     * @return {*}
     */ 
    std::mutex Ebike_det::mtx_t;
    Ebike_det* Ebike_det::instance = nullptr;
    Ebike_det* Ebike_det::getInstance()
    {
        if(instance == nullptr)
        {
            std::unique_lock<std::mutex> lock(mtx_t);
            if(instance == nullptr)
            {
                instance = new Ebike_det();
            }
        }
        return instance;
    }

    telpo_ret_t  Ebike_det::init()
    {
        std::string algCfg = getAlgCfg(std::string("ebike.json"));
        if(telp_det.init(algCfg)!=0)
        {
            std::cout<<"Ebike_det init fail!\n";
        }

        return TELPO_RET_SUCCESS;
    }

    telpo_ret_t Ebike_det::process(cv::Mat &img, std::vector<telpo_object_t> &retObjects)
    {
        if(telp_det.infer(img, retObjects)!=0)
        {
            std::cout<<"Ebike_det process fail!\n";
            return TELPO_RET_FAIL;
        }
        return  TELPO_RET_SUCCESS;
    }

    telpo_ret_t Ebike_det::exit()
    {
        return TELPO_RET_SUCCESS;
    }
...
//Fire_det**************************************************************
     ...
     * @return {*}
     */ 
    std::mutex Fire_det::mtx_t;
    Fire_det* Fire_det::instance = nullptr;
    Fire_det* Fire_det::getInstance()
    {
        if(instance == nullptr)
        {
            std::unique_lock<std::mutex> lock(mtx_t);
            if(instance == nullptr)
            {
                instance = new Fire_det();
            }
        }
        return instance;
    }

    telpo_ret_t Fire_det::init()
    {
        std::string algCfg = getAlgCfg(std::string("fire.json"));
        if(telp_det.init(algCfg)!=0)
        {
            std::cout<<"Fire_det init fail!\n";
        }
...
//Smog_det**************************************************************
     ...
     * @return {*}
     */ 
    std::mutex Smog_det::mtx_t;
    Smog_det* Smog_det::instance = nullptr;
    Smog_det* Smog_det::getInstance()
    {
        if(instance == nullptr)
        {
            std::unique_lock<std::mutex> lock(mtx_t);
            if(instance == nullptr)
            {
                instance = new Smog_det();
            }
        }
        return instance;
    }

    telpo_ret_t Smog_det::init()
    {
        std::string algCfg = getAlgCfg(std::string("smog.json"));
        if(telp_det.init(algCfg)!=0)
        {
            std::cout<<"Smog_det init fail!\n";
        }
